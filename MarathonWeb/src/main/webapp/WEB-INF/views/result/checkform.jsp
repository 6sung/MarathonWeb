...
<% if ("update".equals(request.getParameter("param"))) { %>
	...
	<form action="./update" method="post">
		...
		<input type="button" value="확인" style="background-color:#0d6efd" onclick="return confirmUpdate(this.form)">
	</form>
<!-- 알림창을 표시하는 스크립트 -->
...
<% } %>
 ...
 <% } else if ("delete".equals(request.getParameter("param"))) { %>
	...
	<form action="./delete" method="post">
		접수 번호  <input type="text" name="rcpnum" required><p>
		비밀번호  <input type="password" name="userpassword" required><p>
		<input type="button" value="접수 취소" style="background-color:#0d6efd" onclick="return confirmDelete(this.form)">
	</form>
	<script>
