...
public class MarathonController {
	...
	public String updateReceipt(@PathVariable int receiptNum, @PathVariable String userPassword, Model model, RedirectAttributes redAttr) {
		if (!isValid(receiptNum, userPassword)) {
			...
			return "redirect:/result/checkform?param=update";
		}
	...
	public String updateReceipt(Receipt newReceipt, @RequestParam("receiptNum") int receiptNum, @RequestParam("userPassword") String userPassword, RedirectAttributes model) {
		 try {
		        if (!isValid(receiptNum, userPassword)) {
		            model.addFlashAttribute("message", "접수번호 또는 비밀번호가 다릅니다");
		            return "redirect:/result/checkform?param=update";
		        }
